...
part=$((part+1))
echo "-------------------------===== Section $part =====-------------------------"
echo "Select A DICTIONNARY filename using dialog !"
	echo
	echo Exemples :
	echo
	echo English.dic
	echo "Facebook (Words).dic"
	echo

	dict="$(zenity --file-selection --file-filter="*.txt *.dic" --filename=$HOME/$USER --title="Select a A DICTIONNARY, all format supported")"
...
echo -------------------------========================-------------------------
echo "Select PASSWORD PROTECTED 7Z using dialog !"
...
printf '\033[8;11;105t'		# will resize the window.
...
while read random
	...
	if [ "$?" -eq 0 ]; then
		...
		echo "${green}███████████ PASSWORD FOUND ███████████${reset}"
		echo
		echo Try : $loop PASSWORD IS : $random
		echo
